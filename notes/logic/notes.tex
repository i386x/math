...
\definition Language of propositional logic (plog-lang):
  Let $P$ be a countable nonempty set of symbols, called \term{atomic
  sentences}. The language of the propositional logic, $L_P$, is the set
  containing all symbols from $P$, parenthesis $($ and $)$, and
  \term{propositional connectives} $\lnot$, $\land$, $\lor$, $\imply$, and
  $\iff$.
\quit
